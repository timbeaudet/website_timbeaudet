...
That is essentially what I believe I need to do. _angularVelocity_ should be in radians a second from everything I understand, and _outputTorque_ and _engineInertia_ in Newton-meters (Nm). This would make _engineSpeedRPM_, in revolutions-per-minute to be 60 times angularVelocity over a full rotation (two-pi). This does make sense to my brain, however the results, rate at which the engine speed increases / decreases is not quite what I was expected with no clutch or drive train. It takes a good 3 seconds to go from 1000rpm to 8000rpm and I would be expecting this to be like a second. Will continue tweaking, so stay tuned for updates.
